...
param(
    ...
    [Parameter(Mandatory = $true)][string]$CompiledResource
)
...
public static class KertexResourceApi
{
...
function Read-Identifier([IO.BinaryReader]$Reader) {
...
function Get-IdentifierPointer($Identifier) {
...
$stream = [IO.File]::OpenRead((Resolve-Path -LiteralPath $CompiledResource).Path)
...
try {
    while ($stream.Position -lt $stream.Length) {
        ...
        $type = Read-Identifier $reader
        $name = Read-Identifier $reader
        $stream.Position = $recordStart + $headerSize - 10
        $language = $reader.ReadUInt16()
        $stream.Position = $recordStart + $headerSize
        $data = $reader.ReadBytes($dataSize)
        if ($dataSize -gt 0) {
            $records.Add([PSCustomObject]@{ Type = $type; Name = $name; Language = $language; Data = $data })
        }
    }
} finally {
    $reader.Dispose()
...
$handle = [KertexResourceApi]::BeginUpdateResource($binaryPath, $false)
...
try {
    foreach ($record in $records) {
        ...
        try {
            if (-not [KertexResourceApi]::UpdateResource($handle, $typePointer.Pointer,
                $namePointer.Pointer, $record.Language, $record.Data, $record.Data.Length)) {
                throw "UpdateResource failed: $([Runtime.InteropServices.Marshal]::GetLastWin32Error())"
            }
        } finally {
            if ($typePointer.Allocated) { [Runtime.InteropServices.Marshal]::FreeHGlobal($typePointer.Pointer) }
            if ($namePointer.Allocated) { [Runtime.InteropServices.Marshal]::FreeHGlobal($namePointer.Pointer) }
        }
    }
    $success = $true
} finally {
    if (-not [KertexResourceApi]::EndUpdateResource($handle, -not $success)) {
